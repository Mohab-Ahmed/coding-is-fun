...
* Date   : jan 2020
*
* Description : program from an assignment i did in the coursera
*               C specialization which mimic the game
*               minesweeper but in textual form .
*
********************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
int isBorder(board_t * b, int x, int y)
{
...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME! 
  board_t* data=malloc(sizeof(board_t));
  data->board=malloc(h*sizeof(int*));
  for (int i = 0; i < h; i++)
  {
    data->board[i]=malloc(w*sizeof(int));
    for (int j = 0; j < w; j++)
    {
      data->board[i][j]=UNKNOWN;
    }
  }
  data->height=h;
  data->width=w;
  data->totalMines=numMines;
...
/************************************************
 * 
 * test program to test the 4 functions i wrote.
 * 
 ***********************************************/
//alot of copied code from the program is down here
//cuz basically this was in a seperate file for testing 
/*
struct _board_t {
  int ** board;
  int width;
